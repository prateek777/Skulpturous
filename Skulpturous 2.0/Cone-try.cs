...
{
    public class KinectConeJig : KinectPointCloudJig
    {
        // Our transient solids (cursor sphere & tube) are yellow

        const short transSolColor = 1;

        // Our final solids will be green

        const short finalSolColor = 1;
        ...
        protected override bool WorldDrawData(WorldDraw draw)
        {
            ...
            if (_resizing)
            {
                using (Solid3d cone = new Solid3d())
                {
                    try
                    {
                        cone.CreateFrustum(_profSide,_profSide, _profSide, 0.0);
                    ...
                    using (Solid3d cursor = new Solid3d())
                    {
                        try
                        {
                            ...
                            if (cursor != null)
                            {
                                ...
                                draw.SubEntityTraits.Color = 1; // ColorIndex;
    ...
    public class KinectConeCommands
    {
        [CommandMethod("ADNPLUGINS", "KINEXT4", CommandFlags.Modal)]
        public void ImportFromKinect()
        {
            ...
            KinectConeJig kj =
              new KinectConeJig(doc, tr, 0.1, 10);
